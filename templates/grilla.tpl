{{ define "grilla" }}
  ...
  {{ range $i, $curso := .Cursos }}
    <article
      class="curso-card s-radius-1 s-shadow-bottom background s-shadow-card-micro s-transition white-color nowrap s-column s-mb-0">
      <div class="s-column flex-grow ">
        <div class="img-container s-ratio-16-9 s-radius-tl-1 s-radius-tr-1">
          <a href="/cursos/{{ $curso.Slug }}">
            <img class="img-curso-card s-transition" alt="{{ $curso.Titulo }}"
              src="{{ $curso.Img }}">
          </a>
        </div>
        <div title="{{ $curso.Titulo }}" class="curso-progress s-mt-05"></div>
        <div class="s-pxy-2 s-column flex-grow">
          <a href="/cursos/{{ $curso.Slug }}">
            <h3 class="s-mb-1 t4">
              <span class="s-color-grey-700">{{ $curso.Nombre }}</span>
            </h3>
          </a>
          ...
          <div class="flex nowrap s-to-bottom">
            ...
            <div class="s-cross-center small ">
              <div class="flex nowrap s-cross-center s-mr-1">
                ...
                <p class="s-mb-0 s-color-text-light">{{ $curso.Promedio }}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <footer
        ...
        <div class="s-main-center ">
          <div Titulo="{{ $curso.Profesor }}" class="card__teacher s-cross-center nowrap ">
            <div id="" class="user-avatar flex-none s-small s-mr-1 ">
              <div class="img-container circle">
                <img
                  src="{{ $curso.ProfesorImg }}"
                  alt="Avatar" loading="lazy">
              </div>
            </div>
              <span class="card__teacher-Nombre smaller s-row-text-1 s-z-normal s-mr-1">
                {{ $curso.Profesor }}
              </span>
          </div>
        </div>
